...
function carregarProdutos() {
    fetch('dados.json')
        ...
        .then(data => {
            const container = document.getElementById('produtos-container5');
            ...
            const produtosFiltrados = data.produtos.filter(produto => 
                produto.categoria === 'Feminino' && produto.subcategoria === 'Camisetas'
            );

            // Verificar se há produtos para exibir
            if (produtosFiltrados.length === 0) {
                container.innerHTML = `<p>Nenhum produto encontrado na categoria "Feminino" e subcategoria "Camisetas".</p>`;
                return;
            }

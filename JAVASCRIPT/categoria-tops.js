...
function carregarProdutos() {
    fetch('dados.json')
        ...
        .then(data => {
            const container = document.getElementById('produtos-container9');
            ...
            const produtosFiltrados = data.produtos.filter(produto => 
                produto.categoria === 'Feminino' && produto.subcategoria === 'Tops'
            );

            // Verificar se há produtos para exibir
            if (produtosFiltrados.length === 0) {
                container.innerHTML = `<p>Nenhum produto encontrado na categoria "Feminino" e subcategoria "Tops".</p>`;
                return;
            }

...
function carregarProdutos() {
    fetch('dados.json')
        ...
        .then(data => {
            const container = document.getElementById('produtos-container3');
            ...
            const produtosFiltrados = data.produtos.filter(produto => 
                produto.categoria === 'Masculino' && produto.subcategoria === 'Kits Masculinos'
            );

            // Verificar se há produtos para exibir
            if (produtosFiltrados.length === 0) {
                container.innerHTML = `<p>Nenhum produto encontrado na categoria "Masculino" e subcategoria "Kits Masculinos".</p>`;
                return;
            }
